...
TEST_SEQUENCE("EncodeStrictRefusesWhatItCannotRepresent") {
    const hex::ImHexStringCodec codec;
    ...
    // An algorithmic name is answered by its own encoder alone. "UTF-8" also
    // names an encodings/utf8.tbl table; falling through to it would quietly
    // encode text the strict path just refused.
