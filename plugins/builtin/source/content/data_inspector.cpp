...
namespace hex::plugin::builtin {
    ...
    void registerDataInspectorEntries() {
        ...
        // One code point row per algorithmic Unicode encoding, and one per byte
        // order for UTF-16 and UTF-32 rather than one row following the shared
        // endian toggle: a declared string encoding names "UTF-16LE" or
        // "UTF-16BE" outright.
        //
        // `codeUnitSize` is the least a row needs to show anything: one byte of
        // UTF-8, so a lone ASCII byte at the end of a file still gets a row.
        // `maxSize` is the most one code point ever takes.
        ...
        // UTF-32, the widest encoding decode() supports, spends 4 bytes per code
        // point. This margin holds DisplayBudget code points under any of them.
        ...
        // A declared string encoding names its byte order outright, so this row
        // gets one variant per byte order instead of following the shared
        // endian toggle.
